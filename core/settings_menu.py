...
class Settings(Menu): # inherit from menu class
    ...
    def draw(self, width, height, mouse_released):
        ...
        if self.home_button.draw(self.screen, mouse_released): # display button on screen, checks if it has been clicked
            self.delete_scores_current_index = 0 # set button back to "delete scores"
            self.delete_scores_button.set_text(self.delete_scores_list[self.delete_scores_current_index]) # update button text
